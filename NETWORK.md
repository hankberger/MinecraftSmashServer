...
- Roll only releases compatible with Minecraft 26.2, the current proxy and control protocol **4**. Protocol/Minecraft upgrades require coordinated maintenance. This points release updates the gateway and all backends together; the Mac automatic deployer does that. Deploy the protocol-aware rollout checks from `cc90112` before upgrading an older automatic-deployment installation.
...
The full native-input suite includes mode and party menus, invite acceptance, independent character stages, ready/change/cancel, a two-human duel, party/public FFA filling and leader disconnects. Use `-PmatchmakingTests` for that flow and the winner presentation, or `-PwinnerTests` for only the winner-stage checks. The latter renders every winning class, a four-way draw, different aspect ratios/FOVs, and measures client camera motion and attachment height, plus input gating, exit, character-change handoff and entity cleanup. Pure Java tests cover invitation authority/expiry, stale ready rounds, whole-group queue packing, claim/cancel races and camera framing.
...
### Results, points and rematches (private protocol 4)
...
After a two-second arena result pause, the lobby opens an isolated victory stage. A 1.2-second, fixed-heading pullback frames the winning class model and name, with stats and mouse-clickable replay controls in a separate menu. An invisible mannequin camera rides an empty block display with vanilla `teleport_duration` metadata. The normal entity tracker sends one destination, and the client interpolates the motion locally; the server sends no competing per-tick position corrections. Matching the player's eye height prevents a vertical hop on attachment; neutral movement abilities keep the field of view stable until attachment and are restored on exit. The winning model holds a steady pose after its celebration. Draws present the full roster. The scene uses ordinary vanilla entities and camera packets. The server pack supplies the result panel and button artwork; without the pack, a native button dialog provides the same actions. Neither path needs a client mod. Victory rooms use negative showcase coordinates, separate from character-picker rooms. Sessions own and clean up their models, text and camera on exit, queueing, disconnect and expiry. Controls unlock after the reveal and require a fresh click, preventing held combat input from accepting a rematch. The arena is already reusable while players view the presentation or vote.
...
Main-menu container clicks are scoped to the owning player and current container ID. Only ordinary left/right clicks on action slots execute; shift-click, dragging, dropping, number-key swaps and stale container packets cannot take icons or run actions. Party submenus still use player-bound, one-use dialog tokens.
