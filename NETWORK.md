...
After a two-second arena result pause, the lobby opens an isolated victory stage. A 1.2-second, fixed-heading pullback frames the winning class model, name, stats and world-space replay controls. An invisible mannequin camera rides an empty block display with vanilla `teleport_duration` metadata. The normal entity tracker sends one destination, and the client interpolates the motion locally; the server sends no competing per-tick position corrections. Matching the player's eye height prevents a vertical hop on attachment; neutral movement abilities keep the field of view stable until attachment and are restored on exit. The winning model holds a steady pose after its celebration. Draws present the full roster. These are ordinary vanilla entities and camera packets; no resource pack or client mod is required. Victory rooms use negative showcase coordinates, separate from character-picker rooms. Sessions own and clean up their models, text and camera on exit, queueing, disconnect and expiry. Controls unlock after the reveal and require a fresh click, preventing held combat input from accepting a rematch. The arena is already reusable while players view the presentation or vote.
