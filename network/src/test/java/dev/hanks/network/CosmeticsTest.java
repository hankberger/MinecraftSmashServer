...
class CosmeticsTest {
    ...
    @Test void purchaseEquipsPersistsAndDoesNotReduceLifetimeEarnings() throws Exception {
        ...
        try(var db=new PointsStore(path)) {
            ...
            assertEquals(new PointsStore.Account(75,825,11,11),db.account(player));
        ...
        try(var db=new PointsStore(path)) {
            assertEquals(75,db.account(player).balance());assertEquals(Set.of("diamond"),db.wardrobe(player).owned());
            assertEquals("diamond",db.wardrobes().get(player).equipped("STEVE"));
            assertEquals("default",db.wardrobe(player).equipped("ALEX"));
            fund(db,player,1);assertEquals(150,db.account(player).balance());assertEquals(900,db.account(player).earned());
        }
    }
    ...
    @Test void simultaneousPurchasesAndRetriesCannotOverspendOrChargeTwice() throws Exception {
        var player=UUID.randomUUID();try(var db=new PointsStore(dir.resolve("points.db"));var jobs=Executors.newVirtualThreadPerTaskExecutor()) {
            fund(db,player,11);var writes=new ArrayList<Future<PointsStore.OutfitResult>>();
            ...
            assertEquals(1,purchased);assertEquals(75,db.account(player).balance());assertEquals(1,db.wardrobe(player).owned().size());
        }
    }
    @Test void failureAfterDebitRollsBackBalanceOwnershipAndEquip() throws Exception {
        ...
        try(var db=new PointsStore(file);var external=DriverManager.getConnection("jdbc:sqlite:"+file);var sql=external.createStatement()) {
            ...
            assertEquals(825,db.account(player).balance());assertTrue(db.wardrobe(player).owned().isEmpty());
            sql.execute("DROP TRIGGER fail_equip");assertEquals(PointsStore.OutfitResult.PURCHASED,db.outfit(player,"STEVE","diamond",true));
        }
    }
    @Test void concurrentAffordablePurchasesReceiveOnlyOneDiscount() throws Exception {
        var player=UUID.randomUUID();try(var db=new PointsStore(dir.resolve("points.db"));var jobs=Executors.newVirtualThreadPerTaskExecutor()) {
            fund(db,player,30);
            var steve=jobs.submit(()->db.outfit(player,"STEVE","diamond",true));
            var alex=jobs.submit(()->db.outfit(player,"ALEX","scout",true));
            assertEquals(PointsStore.OutfitResult.PURCHASED,steve.get());assertEquals(PointsStore.OutfitResult.PURCHASED,alex.get());
            assertEquals(0,db.account(player).balance());assertEquals(Set.of("diamond","scout"),db.wardrobe(player).owned());
        }
    }
    ...
    @Test void discountIsUsedOnceAcrossClassesAndStaleCheapQuoteCannotChargeFullPrice() throws Exception {
        var player=UUID.randomUUID();var path=dir.resolve("points.db");
        try(var db=new PointsStore(path)) {
            fund(db,player,30);
            db.outfit(player,"STEVE","diamond",true,750);
            assertEquals(1500,db.account(player).balance());
            assertEquals(PointsStore.OutfitResult.PRICE_CHANGED,db.outfit(player,"ALEX","scout",true,750));
            assertEquals(1500,db.account(player).balance());
        }
        try(var db=new PointsStore(path)) {
            assertEquals(1500,Cosmetics.price(db.wardrobe(player),Cosmetics.skin("ALEX","scout")));
            assertEquals(PointsStore.OutfitResult.PURCHASED,db.outfit(player,"ALEX","scout",true,1500));
            assertEquals(0,db.account(player).balance());assertEquals(2250,db.account(player).earned());
        }
    }
    @Test void legacyPurchasesRemainOwnedAtOriginalPriceWithoutRenewingDiscount() throws Exception {
        var player=UUID.randomUUID();var file=dir.resolve("points.db");
        try(var db=new PointsStore(file);var connection=DriverManager.getConnection("jdbc:sqlite:"+file);var sql=connection.createStatement()) {
            fund(db,player,10);
            sql.execute("INSERT INTO cosmetics VALUES('"+player+"','diamond',250,1)");
            sql.execute("UPDATE accounts SET balance=balance-250 WHERE player='"+player+"'");
        }
        try(var db=new PointsStore(file)) {
            assertEquals(500,db.account(player).balance());assertEquals(PointsStore.OutfitResult.EQUIPPED,db.outfit(player,"STEVE","diamond",false));
            assertEquals(1500,Cosmetics.price(db.wardrobe(player),Cosmetics.skin("ALEX","scout")));
            assertEquals(500,db.account(player).balance());
        }
    }
}
