...
**Fighter Studio.** The packed selector uses `smash_vanilla:showcase`, a separate dimension. `ShowcaseBuilder.ensureFighterBuilt` creates a continuous gray floor and wall with stone inlays, an octagonal copper/quartz backdrop with inset lights, and a chamfered quartz/stone presentation dais. The camera is closer, at `(-4.5, 104.5, 13)` relative to the room, framing the full fighter on the right of the four-column menu. The backdrop is offset along the camera sightline so it centers behind the fighter in perspective. There are no roster shelves, tree canopies or island roots. A diamond-block marker at `(0,93,0)` versions the new studio: the first packed visit clears the combined old/new set bounds and rebuilds, including existing saved gardens. Winner rooms use negative room IDs and retain their garden scenery and camera animation. The resource-pack-disabled fallback still rebuilds its old garden set when used.
