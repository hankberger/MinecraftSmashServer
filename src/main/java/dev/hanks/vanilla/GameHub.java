...
public final class GameHub {
    ...
    public GameHub(VanillaSmash game) { this.game = game; }
    public void reset() { parties.clear(); absentSince.clear(); notices.clear(); noticeUntil.clear(); localRound = null; for (var t : localQueue.tickets()) localQueue.remove(t.player()); }
    ...
    public void chooseDirect(ServerPlayer p, FighterClass fighter, VanillaSmash.Mode mode) {
        ensure(p); var round = parties.start(p.getUUID(), mode.name()); confirm(p, fighter, round);
    }
    ...
    public void disconnected(ServerPlayer p) {
        menu.forget(p.getUUID()); absentSince.put(p.getUUID(), game.ticks);
    ...
    public void presence(Set<UUID> online) {
        for (var id : parties.players()) {
            ...
            if (game.ticks - since >= 40) { var view = parties.view(id); parties.disconnect(id); absentSince.remove(id); refresh(view); }
        }
    }
    ...
    private void accept(ServerPlayer p, UUID party) {
        parties.accept(p.getUUID(), party, game.ticks); notices.remove(p.getUUID()); noticeUntil.remove(p.getUUID());
    ...
    private void memberMenu(ServerPlayer p, PartyBook.Member member) {
        menu.show(p, member.name(), "", List.of(
                ...
                button(p, "Remove from party", () -> { if (!cancel(p, false)) return; parties.kick(p.getUUID(), member.id()); var other = player(member.id()); if (other != null) open(other); open(p); refresh(parties.view(p.getUUID())); })
        ), false, () -> manageMenu(p));
    }
    private void leaveParty(ServerPlayer p) {
        var previous = ensure(p); if (!cancel(p, true)) return;
        parties.leave(p.getUUID()); open(p); refresh(previous);
    }
