...
public final class MvpWorlds {
    ...
    public static boolean managed(Level level) { return level.dimension().equals(LOBBY) || level.dimension().equals(ARENA) || level.dimension().equals(SHOWCASE); }
    public static void prepare(MinecraftServer server, boolean buildLobby, boolean buildArena) {
        ...
        for (var level : new net.minecraft.server.level.ServerLevel[]{lobby, arena, showcase}) {
