...
public final class PointsClientTest {
    ...
    public static void run(ClientGameTestContext c) {
        ...
        try(var server=c.worldBuilder().createServer(props);var connection=server.connect()) {
            ...
            server.runOnServer(s->game().match.finish(id,"Points fixture"));
            ...
            server.waitFor(s->game().points.account(id).balance()==150 && game().hub.results.book.result(id)!=null,200);
            server.runOnServer(s->{
            ...
            server.waitFor(s->game().points.account(id).balance()==225 && game().battle==null,200);
            ...
            server.runOnServer(s->{
                ...
                check(game().points.account(id).balance()==225 && game().points.account(id).matches()==3,"Balance reloads from persistent account after store restart");
            ...
            server.runOnServer(s->{check(game().points.account(id).balance()==225,"Practice cannot farm currency");friends.forEach(MatchmakingClientTest.Peer::leave);});
        }
