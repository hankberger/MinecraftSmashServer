...
**Points:** finishing a qualifying multiplayer match earns **50**, plus **25** for a win. Draws earn 50 each. Practice and leaving while still fighting earn none; leaving after losing your final stock still counts as finishing. Balances persist across restarts and arena transfers. The lobby and character picker show your balance, results show your reward, and `/smash points` shows your balance and lifetime earnings. Skins cost **1,500 points**, with a one-time **750-point first purchase**; all five starter fighters remain free. Rewards require a match lasting at least 30 seconds and five seconds of active participation. See [POINTS.md](POINTS.md) for reward rules, persistence and operations.
...
Combat feedback scales with impact: short class-colored contact sparks, layered fighter sounds with subtle left/right positioning, and 1/2/3-tick impact pauses for light/heavy/strong-launch hits. A distant projectile or trap never pauses its owner. Damage numbers pulse immediately and turn warmer at high percentages. Strong launches leave player-colored smoke trails; blocks spark, broken shields shatter, and off-camera KOs still show a brief HUD callout. Jumps, double jumps, landings and recovery have short sound/particle cues. Countdown beats lead into GO, and the final stock ends with GAME! Effects add no damage or hitboxes, and contact displays expire within 350 ms with a fixed simultaneous-effect cap.
...
| Zombie | Quick claws with a baby zombie companion that echoes attacks. Charge a tandem swipe, or use F to throw the baby forward. Opponents can temporarily knock out the partner. | Two native zombie models, synchronized windup poses, green claw trails and a small partner swipe |
| Skeleton | Bone melee pushes opponents back; Scatter Retreat creates distance for a charged bow shot. Arrows retain normal gravity and charge-dependent range. | Held bone, native bow draw and real arrows |
| Villager | Wooden axe melee, sapling traps, impact bells and a golem summon. Thrown bells ring immediately on an opponent or become timed traps after landing. | Wooden axe, bells, growing saplings, flowerpots and an iron golem |
...
Descending near either end of the main deck automatically catches the ledge, including after a spent recovery. Hold toward the stage to climb, press Space to jump off, or hold S to drop/bypass a catch. Grabs cannot cancel hit stun. The first grab grants 0.5 seconds of protection; leaving ends it. Hanging lasts at most two seconds, recatching has a 0.6-second delay, and only two grabs are available before landing. A second grab has no protection. A new arrival displaces an existing hanger. Hanging and ledge jumps never refill air resources; climbing onto the deck or landing does. The side blast zones sit 22 blocks beyond the deck and the bottom zone is 28 blocks below it, leaving space to recover without changing jump height, gravity or recovery strength.
...
./gradlew.bat --gradle-user-home ../smash_arena/.gradle-user-home runClientGameTest -PdedicatedTests -PplaytestTests
...
### Multiplayer playtest tuning

Normal combat speed is 24% lower, with weaker air steering and a smaller Alex speed advantage. Jump height still clears the stage platforms. S waits 250 ms before dropping/fast-falling; an accepted down attack suppresses both for that press. Fast-fall builds gradually instead of snapping downward.

Party dialog Back buttons return to their parent screen. Leaving a party selection cancels its queue and returns every member to the lobby, without reopening Play on their peers. Falling off the lobby returns to the courtyard spawn. Arena camera controllers are hidden from other clients and parked behind the furthest camera position so player bodies cannot obstruct the match.
